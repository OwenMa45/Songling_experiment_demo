...
def load(path=None):
    with open(path or ROOT / "configs/server.yaml", encoding="utf-8") as f:
    ...
    if s["control_hz"] <= 0 or abs(ratio - round(ratio)) > 1e-8:
        raise ValueError("control period must be a positive integer multiple of timestep")
